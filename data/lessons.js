...
const TOPIC_DAYS = [
  {
    ...
    concepts: [
      c('What SQL Asks For', 'SQL 在问什么', 'SQL means Structured Query Language.', 'SQL 全称是 Structured Query Language（结构化查询语言）。', 'SQL stands for Structured Query Language. A database stores related facts in tables. A query does not change the stored data unless it is a data-changing statement; a SELECT builds a temporary result set from those facts. In this course, employees, departments, customers, products, orders, and order_items are small enough to inspect directly, so use SELECT as a microscope before you write more complex logic.', 'SQL 全称是 Structured Query Language。数据库把相关事实放在表中。查询通常不会改变存储的数据；SELECT 只是从事实中临时组装一个结果集。本课程的 employees、departments、customers、products、orders、order_items 都很小，适合先用 SELECT 当显微镜观察，再写复杂逻辑。', 'SELECT * FROM employees;', 'Before solving a problem, name the table that contains the grain you need.', '解题前先说清楚：答案的一行应该来自哪张表、代表什么粒度。'),
  ...
  },
  {
    ...
    concepts: [
      ...
      c('Preview: WHERE vs HAVING', '预告：WHERE 与 HAVING', 'WHERE filters rows; HAVING filters groups after GROUP BY.', 'WHERE 过滤行；HAVING 在 GROUP BY 之后过滤组。', 'You only need WHERE today. Later, when you group rows, HAVING will filter whole groups using aggregates such as COUNT or AVG. Remember the contrast early: WHERE cannot use AVG(salary), because aggregates do not exist until after grouping.', '今天只要掌握 WHERE。之后学习分组时，HAVING 会用 COUNT、AVG 等聚合过滤整组。先记住对比：WHERE 不能写 AVG(salary)，因为聚合值要等分组后才存在。', "-- Today\nSELECT * FROM employees WHERE salary > 80000;\n-- Later (Day 15+)\n-- ... GROUP BY dept_id HAVING AVG(salary) > 75000;", 'Does your condition test one row or one group summary?', '你的条件测的是一行，还是一个组的汇总？'),
    ],
  ...
  },
  {
    ...
    concepts: [
      ...
      c('NULL Is Unknown', 'NULL 是未知', 'NULL is not zero and not an empty string.', 'NULL 不是 0，也不是空字符串。', 'NULL means the value is missing or unknown. Comparisons such as salary = NULL do not return true; use IS NULL or IS NOT NULL. This matters in employees because Jack has a missing salary. Preview for later aggregates: COUNT(salary) ignores NULL salaries, while COUNT(*) still counts the row.', 'NULL 表示值缺失或未知。salary = NULL 不会得到 true；应使用 IS NULL 或 IS NOT NULL。employees 中 Jack 的 salary 缺失，适合观察这一点。后面聚合也会用到：COUNT(salary) 会忽略 NULL 工资，而 COUNT(*) 仍会计入该行。', 'SELECT first_name, salary\nFROM employees\nWHERE salary IS NULL;', 'Would an unknown value pass a normal equality test?', '未知值能通过普通等值判断吗？'),
      c('COALESCE for Defaults', 'COALESCE 默认值', 'COALESCE returns the first non-NULL expression and needs at least two arguments.', 'COALESCE 返回第一个非 NULL 表达式，至少需要两个参数。', 'Use COALESCE when the display or calculation needs a fallback. COALESCE(a, b) or COALESCE(a, b, c) works; a single-argument COALESCE is not useful. Be careful: replacing NULL salary with 0 changes analytical meaning, so label it clearly and avoid hiding missingness when data quality matters.', '当展示或计算需要兜底值时使用 COALESCE。COALESCE(a, b) 或 COALESCE(a, b, c) 都可以；只写一个参数没有意义。注意：把 NULL salary 替换成 0 会改变分析含义，因此要明确标注，不要在数据质量重要时掩盖缺失。', "SELECT first_name,\n       COALESCE(salary, 0) AS salary_for_display\nFROM employees;", 'Are you fixing display, or changing analytical meaning?', '你是在修复展示，还是改变分析含义？'),
    ],
  ...
  },
  {
    ...
    concepts: [
      c('SQLite Type Affinity', 'SQLite 类型亲和性', 'Declared types are preferences, not hard locks.', '声明类型更像偏好，不是硬锁。', 'SQLite uses dynamic typing with type affinity: a column declared INTEGER prefers integers, but the engine may still store other values unless constraints stop them. Still, treat salary and unit_price as numeric, hire_date and order_date as ISO text dates, and ids as integers so your SQL transfers to stricter systems.', 'SQLite 使用动态类型，并带有类型亲和性：声明为 INTEGER 的列更偏好整数，但除非约束阻止，引擎仍可能存入其他值。实践中仍把 salary、unit_price 当数值，hire_date、order_date 当 ISO 文本日期，id 当整数，这样 SQL 更容易迁移到严格系统。', 'SELECT typeof(salary), typeof(hire_date)\nFROM employees\nLIMIT 3;', 'What type does the operation expect?', '这个操作期待什么类型？'),
      ...
      c('INTEGER PRIMARY KEY in SQLite', 'SQLite 中的 INTEGER PRIMARY KEY', 'INTEGER PRIMARY KEY often acts as the row id.', 'INTEGER PRIMARY KEY 常等同于行 ID。', 'In SQLite, a column declared INTEGER PRIMARY KEY becomes an alias for the internal rowid. That is why many teaching schemas use INTEGER PRIMARY KEY for emp_id, order_id, and product_id. It is both a unique identifier and a convenient row handle.', '在 SQLite 中，声明为 INTEGER PRIMARY KEY 的列会成为内部 rowid 的别名。这也是许多教学 schema 用 INTEGER PRIMARY KEY 作为 emp_id、order_id、product_id 的原因：它既是唯一标识，也是方便的行把手。', 'CREATE TABLE demo (\n  demo_id INTEGER PRIMARY KEY,\n  label TEXT\n);', 'Is the primary key identifying the row, or also acting as SQLite rowid?', '这个主键是在标识行，还是同时也充当 SQLite rowid？'),
    ],
  ...
  },
  {
    ...
    concepts: [
      c('SQL Statement Families', 'SQL 语句家族：DML / DDL / DCL / TCL', 'Common teaching taxonomies classify SQL statements into families.', '常见教学分类会把 SQL 语句分成几大家族。', 'DML (Data Manipulation Language) works with row data: SELECT, INSERT, UPDATE, DELETE. In many textbooks and exam questions, SELECT is counted as DML even though it does not change data. DDL (Data Definition Language) changes structure: CREATE, ALTER, DROP. DCL (Data Control Language) manages privileges: GRANT, REVOKE. TCL (Transaction Control Language) controls commit boundaries: BEGIN/COMMIT/ROLLBACK/SAVEPOINT. Remember the map before you classify a statement on a quiz.', 'DML（数据操纵语言）处理行数据：SELECT、INSERT、UPDATE、DELETE。很多教材和考题仍把 SELECT 算作 DML，即使它不改数据。DDL（数据定义语言）改变结构：CREATE、ALTER、DROP。DCL（数据控制语言）管理权限：GRANT、REVOKE。TCL（事务控制语言）控制提交边界：BEGIN/COMMIT/ROLLBACK/SAVEPOINT。做分类题前先记住这张地图。', '-- DML: SELECT / INSERT / UPDATE / DELETE\n-- DDL: CREATE / ALTER / DROP\n-- DCL: GRANT / REVOKE\n-- TCL: COMMIT / ROLLBACK / SAVEPOINT', 'Which family does this statement belong to, and why?', '这句话属于哪个家族，为什么？'),
      c('INSERT with Column Lists', '带列清单 INSERT', 'Column lists protect you from schema-order mistakes.', '列清单可以避免列顺序错误。', 'INSERT INTO table (columns) VALUES (...) is clearer than relying on the table column order. In learning databases, ids may be manual; in production, many ids are generated automatically. The habit that matters is naming the columns you are supplying. If you violate NOT NULL, PRIMARY KEY, UNIQUE, or FOREIGN KEY, the INSERT fails—constraints reject bad rows instead of silently accepting them.', 'INSERT INTO table (columns) VALUES (...) 比依赖表列顺序更清楚。学习库里 id 可能手动填写；生产中很多 id 自动生成。关键习惯是写出你提供了哪些列。如果违反 NOT NULL、PRIMARY KEY、UNIQUE 或 FOREIGN KEY，INSERT 会失败——约束会拒绝坏数据，而不是默默接受。', "INSERT INTO customers (customer_id, name, city, country)\nVALUES (6, 'Wayne Enterprises', 'Gotham', 'USA');", 'What constraint could reject this row?', '哪种约束可能拒绝这行？'),
      ...
      c('DELETE Is Not DROP', 'DELETE 不是 DROP', 'DELETE removes rows; DROP removes the object.', 'DELETE 删除行，DROP 删除对象。', 'DELETE FROM orders WHERE status = cancelled removes matching rows but leaves the orders table. DROP TABLE orders removes the table definition and data. Use DELETE with a WHERE clause and verify row counts carefully. DELETE is DML; DROP is DDL.', 'DELETE FROM orders WHERE status = cancelled 删除匹配行，但保留 orders 表。DROP TABLE orders 删除表定义和数据。使用 DELETE 时要带 WHERE 并仔细确认行数。DELETE 属于 DML；DROP 属于 DDL。', "DELETE FROM orders\nWHERE status = 'cancelled' AND order_id = 107;", 'Would a SELECT with the same WHERE show only disposable rows?', '同样 WHERE 的 SELECT 是否只显示可删除行？'),
    ],
    pitfalls: { en: ['Preview UPDATE and DELETE targets with SELECT first.', 'Foreign keys may reject inserts or deletes.', 'On quizzes, SELECT is usually classified as DML.'], zh: ['UPDATE/DELETE 前先用 SELECT 预览目标。', '外键可能拒绝插入或删除。', '考题中 SELECT 通常归入 DML。'] },
  ...
  },
  {
    ...
    concepts: [
      c('DDL Defines Shape', 'DDL 定义形状', 'CREATE TABLE describes what rows are allowed to look like.', 'CREATE TABLE 描述允许存入什么样的行。', 'DDL statements change database structure: CREATE, ALTER, and DROP. A good table definition names the entity, gives every row an identifier, and marks required facts with NOT NULL. Think of schema design as writing rules for future data. Contrast with Day 7: INSERT/UPDATE/DELETE are DML; they change rows, not the table definition.', 'DDL 语句改变数据库结构：CREATE、ALTER、DROP。好的表定义会命名实体、给每行标识符，并用 NOT NULL 标记必需事实。可以把 schema 设计看成给未来数据写规则。对比第 7 天：INSERT/UPDATE/DELETE 是 DML，它们改行，不改表定义。', 'CREATE TABLE IF NOT EXISTS suppliers (\n  supplier_id INTEGER PRIMARY KEY,\n  name TEXT NOT NULL,\n  contact_email TEXT\n);', 'What does one row in this new table mean?', '新表的一行代表什么？'),
      c('Defaults, UNIQUE, and Required Fields', '默认值、UNIQUE 与必填字段', 'Constraints make bad data harder to store.', '约束让坏数据更难进入数据库。', 'NOT NULL says a value must be present. DEFAULT supplies a value when the insert omits it. UNIQUE enforces uniqueness on a column (SQLite may allow multiple NULLs in a UNIQUE column). A table has one PRIMARY KEY, which is UNIQUE and NOT NULL, but it may have many UNIQUE constraints. Example: label TEXT UNIQUE.', 'NOT NULL 表示必须有值。DEFAULT 在插入省略该列时提供默认值。UNIQUE 保证列值唯一（SQLite 的 UNIQUE 列可能允许多个 NULL）。一张表只有一个 PRIMARY KEY（唯一且非空），但可以有多个 UNIQUE 约束。例如：label TEXT UNIQUE。', "CREATE TABLE IF NOT EXISTS tags (\n  tag_id INTEGER PRIMARY KEY,\n  label TEXT UNIQUE,\n  active INTEGER DEFAULT 1\n);", 'Which fields are truly required at creation time?', '创建记录时哪些字段真的必须存在？'),
      c('ALTER and DROP TABLE', 'ALTER 与 DROP TABLE', 'ALTER changes a living table; DROP removes it.', 'ALTER 修改已有表；DROP 删除表。', 'ALTER TABLE is DDL used to change an existing definition (SQLite support is limited but includes adding columns and some renames). DROP TABLE removes the table and its data. In a practice browser database, DROP is recoverable by refreshing; in real systems it can be catastrophic. Prefer IF EXISTS for repeatable scripts.', 'ALTER TABLE 属于 DDL，用于修改已有定义（SQLite 支持有限，但可加列和部分重命名）。DROP TABLE 删除表及其数据。在浏览器练习库里可通过刷新恢复；在真实系统中可能是灾难。可用 IF EXISTS 写可重复脚本。', 'ALTER TABLE product_notes ADD COLUMN author TEXT;\nDROP TABLE IF EXISTS product_notes;', 'Are you changing structure, or deleting the whole object?', '你是在改结构，还是删除整个对象？'),
    ],
    pitfalls: { en: ['SQLite accepts many type names but does not enforce them like stricter databases.', 'DROP TABLE is structural deletion, not row deletion.', 'UNIQUE is not the same as PRIMARY KEY.'], zh: ['SQLite 接受很多类型名，但不像严格数据库那样强制。', 'DROP TABLE 是结构删除，不是行删除。', 'UNIQUE 不等于 PRIMARY KEY。'] },
  ...
  },
  {
    ...
    concepts: [
      c('Primary Keys', '主键', 'A primary key identifies one row.', '主键标识唯一一行。', 'employees.emp_id, orders.order_id, and products.product_id are primary keys. They are stable handles for rows, better than names because names can repeat or change. Use keys when joining or updating a specific row. A composite primary key uses more than one column, written PRIMARY KEY (col1, col2). Uniqueness then applies to the combination, not each column alone.', 'employees.emp_id、orders.order_id、products.product_id 都是主键。它们是行的稳定把手，比姓名更适合，因为姓名可能重复或变化。JOIN 或修改特定行时应使用键。复合主键使用多列，写作 PRIMARY KEY (col1, col2)；此时唯一性针对组合，不是单独每一列。', 'SELECT emp_id, first_name, last_name\nFROM employees\nORDER BY emp_id;\n-- Composite example:\n-- PRIMARY KEY (order_id, product_id)', 'What column uniquely identifies this row?', '哪一列唯一标识这一行？'),
  ...
  },
  {
    ...
    concepts: [
      c('One-to-Many', '一对多', 'One parent row can relate to many child rows.', '一个父行可以关联多个子行。', 'One department can have many employees. One customer can have many orders. One order can have many order_items. Recognizing one-to-many relationships helps you predict whether a JOIN will duplicate rows. Products and orders are many-to-many: one order has many products, and one product appears in many orders. The junction (bridge) table order_items stores that relationship.', '一个部门可以有多名员工；一个客户可以有多张订单；一张订单可以有多条明细。识别一对多关系可以帮助预测 JOIN 后行数是否会增加。商品与订单是多对多：一张订单有多种商品，一种商品出现在多张订单中。中间表（桥接表）order_items 保存这种关系。', 'SELECT dept_id, COUNT(*)\nFROM employees\nGROUP BY dept_id;', 'Which table is the parent, and which is the child?', '哪张表是父表，哪张表是子表？'),
      ...
      c('Normalize Then Recombine', '规范化后再组合', 'Databases avoid repetition, queries rebuild context.', '数据库避免重复，查询负责重建上下文。', 'Storing dept_name only in departments avoids update anomalies. A query then recombines employees with departments when you need a human-readable report. This is the central trade-off of relational design. The opposite pattern—repeating dept_name on every employee row—is denormalization (反范式) and invites inconsistent updates.', 'dept_name 只存在 departments 中，可以避免更新异常。需要人类可读报表时，再通过查询把 employees 和 departments 组合起来。这是关系设计的核心权衡。相反做法——在每个员工行重复 dept_name——属于反范式，容易造成不一致更新。', 'SELECT emp_id, first_name, dept_id\nFROM employees\nWHERE dept_id = 1;\n\nSELECT dept_name\nFROM departments\nWHERE dept_id = 1;', 'What repeated data is normalization avoiding?', '规范化避免了哪些重复数据？'),
    ],
  ...
  },
  {
    ...
    concepts: [
      ...
      c('Comma FROM Is Old-Style Join', '逗号 FROM 是旧式连接', 'FROM a, b without a WHERE link is a Cartesian product.', 'FROM a, b 若没有 WHERE 关联条件，就是笛卡尔积。', 'Older SQL writes FROM employees e, departments d and puts the match in WHERE e.dept_id = d.dept_id. That is roughly equivalent to CROSS JOIN plus a filter, or to INNER JOIN when the WHERE is complete. Prefer modern JOIN ... ON for clarity. Missing the WHERE condition multiplies every left row by every right row.', '旧式 SQL 写作 FROM employees e, departments d，并把匹配条件放在 WHERE e.dept_id = d.dept_id。这大致等价于 CROSS JOIN 再过滤；WHERE 完整时也像 INNER JOIN。为清晰起见优先写 JOIN ... ON。缺少 WHERE 关联时，左表每行会与右表每行组合。', 'SELECT e.first_name, d.dept_name\nFROM employees e, departments d\nWHERE e.dept_id = d.dept_id;\n-- Dangerous without WHERE: every employee with every department', 'Is every table pair linked by an ON or WHERE condition?', '每对表是否都有 ON 或 WHERE 关联条件？'),
    ],
    pitfalls: { en: ['Self joins require distinct aliases.', 'Join from the table that matches the desired result grain.', 'FROM a, b without a link condition creates a Cartesian product.'], zh: ['自连接必须使用不同别名。', '从符合目标粒度的表开始 JOIN。', 'FROM a, b 缺少关联条件会产生笛卡尔积。'] },
  ...
  },
  {
    ...
    concepts: [
      ...
      c('EXISTS and Correlated Subqueries', 'EXISTS 与相关子查询', 'EXISTS tests whether any matching row exists; correlated subqueries refer to the outer row.', 'EXISTS 测试是否存在匹配行；相关子查询会引用外层行。', 'EXISTS (SELECT 1 FROM ...) is true when the subquery finds at least one row. It cares about existence, not the count. A correlated subquery references outer columns (for example e.dept_id) and is conceptually evaluated per outer row. Non-correlated subqueries usually run once and feed a fixed value or list to the outer query.', 'EXISTS (SELECT 1 FROM ...) 在子查询至少找到一行时为真；它关心“有没有”，不关心具体行数。相关子查询引用外层列（如 e.dept_id），概念上对每一行外层再算一次。非相关子查询通常先算一次，再把固定值或列表交给外层。', 'SELECT e.first_name, e.salary\nFROM employees e\nWHERE e.salary > (\n  SELECT AVG(salary) FROM employees e2 WHERE e2.dept_id = e.dept_id\n);\n\nSELECT c.name\nFROM customers c\nWHERE EXISTS (\n  SELECT 1 FROM orders o WHERE o.customer_id = c.customer_id\n);', 'Does the subquery mention a column from the outer query?', '子查询是否引用了外层查询的列？'),
      c('ALL and ANY Comparisons', 'ALL 与 ANY 比较', 'ALL and ANY compare one value against a multi-row subquery.', 'ALL 和 ANY 用一个值与多行子查询比较。', 'x > ALL (subquery) means x is greater than every returned value. x > ANY (subquery) means x is greater than at least one returned value. These are advanced comparison patterns for multi-row subqueries; many problems can also be rewritten with MIN/MAX or EXISTS.', 'x > ALL (子查询) 表示 x 大于返回的每一个值；x > ANY (子查询) 表示 x 至少大于其中一个值。这是多行子查询的高级比较；很多题目也可用 MIN/MAX 或 EXISTS 改写。', 'SELECT first_name, salary\nFROM employees\nWHERE salary > ALL (\n  SELECT salary FROM employees WHERE dept_id = 2 AND salary IS NOT NULL\n);', 'Do you need greater than every value, or greater than some value?', '你需要大于每一个值，还是大于某一个值？'),
    ],
    pitfalls: { en: ['Scalar subqueries must not return multiple rows.', 'A FROM subquery needs an alias.', 'EXISTS checks presence, not how many rows matched.'], zh: ['标量子查询不能返回多行。', 'FROM 子查询需要别名。', 'EXISTS 检查是否存在，不检查匹配行数。'] },
  ...
  },
  {
    ...
    concepts: [
      ...
      c('GROUP BY Ordinal Positions', 'GROUP BY 位置编号', 'Some engines allow GROUP BY 1 to mean the first SELECT column.', '有些引擎允许 GROUP BY 1 表示按 SELECT 第一列分组。', 'In SQLite, GROUP BY 1 groups by the first expression in the SELECT list. It is short, but fragile: reordering SELECT columns silently changes the grouping. Prefer GROUP BY dept_id or an explicit expression for portable, readable SQL. Know the ordinal form so quiz questions make sense.', '在 SQLite 中，GROUP BY 1 按 SELECT 列表第一个表达式分组。写法短，但不稳健：调换 SELECT 列会悄悄改变分组。可移植、可读的写法应明确写 GROUP BY dept_id 或表达式。了解位置编号是为了能答对相关考题。', 'SELECT dept_id, COUNT(*) AS headcount\nFROM employees\nGROUP BY 1;', 'Would the grouping still be obvious if SELECT columns were reordered?', '如果调换 SELECT 列顺序，分组意图是否仍然清楚？'),
    ],
    pitfalls: { en: ['Do not select ungrouped detail columns in grouped queries.', 'WHERE filters rows, not groups.', 'Prefer named GROUP BY columns over ordinals in real work.'], zh: ['分组查询中不要选择未分组的明细列。', 'WHERE 过滤行，不过滤组。', '实际工作中优先用列名 GROUP BY，而不是位置编号。'] },
  ...
  },
  {
    ...
    concepts: [
      ...
      c('INTERSECT and EXCEPT', 'INTERSECT 与 EXCEPT', 'Set operators also include intersection and difference.', '集合运算还包括交集与差集。', 'INTERSECT returns rows that appear in both SELECT results. EXCEPT (or MINUS in some dialects) returns rows in the first result but not the second. SQLite supports INTERSECT and EXCEPT; INTERSECT became broadly available in recent versions (for example 3.39+). Like UNION, both sides need matching shapes.', 'INTERSECT 返回两个 SELECT 结果中都出现的行。EXCEPT（有些方言叫 MINUS）返回只在第一段结果中出现的行。SQLite 支持 INTERSECT 与 EXCEPT；INTERSECT 在较新版本（如 3.39+）更常见。与 UNION 一样，两边结果形状必须匹配。', "SELECT city AS place FROM customers\nINTERSECT\nSELECT location AS place FROM departments;", 'Do you need rows in either set, both sets, or only the first set?', '你需要任一侧、两侧都有，还是只在第一侧的行？'),
    ],
    pitfalls: { en: ['Column counts must match.', 'ORDER BY belongs at the end of the compound query.', 'INTERSECT keeps only rows present in both sides.'], zh: ['列数必须匹配。', 'ORDER BY 属于复合查询末尾。', 'INTERSECT 只保留两侧都有的行。'] },
  ...
  },
  {
    ...
    concepts: [
      ...
      c('Window Aggregates and Filtering', '窗口聚合与过滤', 'SUM() OVER and AVG() OVER are window aggregates; filter them via subquery or CTE.', 'SUM() OVER、AVG() OVER 属于窗口聚合；过滤它们需用子查询或 CTE。', 'Window aggregates keep detail rows while adding running or partitioned totals, for example SUM(salary) OVER (PARTITION BY dept_id). SQLite has supported window functions since 3.25+. Because window results are calculated after WHERE, you cannot put ROW_NUMBER() directly in WHERE. Wrap the query in a subquery or CTE, then filter on the computed column.', '窗口聚合在保留明细行的同时增加累计或分区合计，例如 SUM(salary) OVER (PARTITION BY dept_id)。SQLite 自 3.25+ 支持窗口函数。窗口结果在 WHERE 之后计算，因此不能把 ROW_NUMBER() 直接写进 WHERE。应先用子查询或 CTE 包一层，再按计算列过滤。', 'SELECT *\nFROM (\n  SELECT first_name, salary,\n         AVG(salary) OVER (PARTITION BY dept_id) AS dept_avg\n  FROM employees\n) ranked\nWHERE salary > dept_avg;', 'Are you aggregating with GROUP BY collapse, or with OVER keep-detail?', '你是用 GROUP BY 压缩行，还是用 OVER 保留明细？'),
    ],
    pitfalls: { en: ['Window functions cannot be used directly in WHERE.', 'OVER ORDER BY is not the same as final ORDER BY.', 'Filter window results with a subquery or CTE.'], zh: ['窗口函数不能直接用于 WHERE。', 'OVER 内 ORDER BY 不等于最终 ORDER BY。', '用子查询或 CTE 过滤窗口结果。'] },
  ...
  },
  {
    ...
    concepts: [
      ...
      c('Recursive CTE and MATERIALIZED', '递归 CTE 与 MATERIALIZED', 'Know the advanced options; you do not need them for most course tasks.', '了解高级选项即可；本课程多数任务用不到。', 'A recursive CTE uses WITH RECURSIVE to walk hierarchies, such as manager chains. SQLite also accepts optional MATERIALIZED / NOT MATERIALIZED hints that suggest whether a CTE should be computed once and stored. Treat these as advanced awareness items: recognize them on quizzes, prefer ordinary CTEs for day-to-day course work.', '递归 CTE 使用 WITH RECURSIVE 遍历层级，例如经理汇报链。SQLite 还接受可选的 MATERIALIZED / NOT MATERIALIZED 提示，建议是否先计算并物化 CTE。把它们当作进阶认知：考题能认出，日常本课程仍优先用普通 CTE。', '-- Awareness only:\n-- WITH RECURSIVE ... AS (... UNION ALL ...)\n-- WITH cte AS MATERIALIZED (SELECT ...)', 'Is the problem asking for hierarchy walking, or just a readable named step?', '题目是要遍历层级，还是只要可读的命名步骤？'),
    ],
    pitfalls: { en: ['CTE names must be unique within one WITH clause.', 'A CTE is scoped to the single statement.', 'MATERIALIZED is an optional optimization hint, not required syntax.'], zh: ['同一个 WITH 中 CTE 名不能重复。', 'CTE 只在单条语句内有效。', 'MATERIALIZED 是可选优化提示，不是必写语法。'] },
  ...
  },
  {
    ...
    concepts: [
      ...
      c('Trade-Offs', '权衡', 'Indexes speed reads but add write and storage cost.', '索引加速读取，但增加写入和存储成本。', 'Every INSERT, UPDATE, or DELETE may also update indexes. Too many indexes can slow writes and complicate planning. Index design should come from observed query patterns, not from indexing every column. Composite indexes cover multiple columns in order, for example CREATE INDEX idx_emp_dept_sal ON employees(dept_id, salary). Drop unused indexes with DROP INDEX IF EXISTS name.', '每次 INSERT、UPDATE、DELETE 都可能同时维护索引。过多索引会拖慢写入并增加优化复杂度。索引设计应来自观察到的查询模式，而不是给每列都建索引。复合索引按顺序覆盖多列，例如 CREATE INDEX idx_emp_dept_sal ON employees(dept_id, salary)。不用的索引用 DROP INDEX IF EXISTS name 删除。', 'CREATE INDEX idx_emp_dept_sal ON employees(dept_id, salary);\nDROP INDEX IF EXISTS idx_employees_dept_id;\nEXPLAIN QUERY PLAN\nSELECT * FROM employees WHERE dept_id = 1;', 'What query pattern justifies the index?', '什么查询模式证明这个索引值得建？'),
    ],
    pitfalls: { en: ['Do not index every column by default.', 'Low-cardinality columns may not help much alone.', 'Composite index column order matters for lookups.'], zh: ['不要默认给每列建索引。', '低基数列单独建索引收益可能有限。', '复合索引的列顺序影响查找效果。'] },
  ...
  },
  {
    ...
    concepts: [
      ...
      c('View Limitations', '视图限制', 'Views are not automatic performance magic.', '视图不会自动带来性能魔法。', 'A standard view usually re-runs its definition. Some databases support materialized views, but that is a different object. SQLite views can be limited for updates, so treat them mainly as readable query abstractions here. Views can also hide columns for a simpler or safer interface: expose only the columns a role needs.', '标准视图通常会重新运行其定义。有些数据库支持物化视图，但那是不同对象。SQLite 视图在更新方面有限，本课程主要把它当作可读查询抽象。视图也可以隐藏列，提供更简单或更安全的接口：只暴露某个角色需要的列。', 'DROP VIEW IF EXISTS v_employee_dept;', 'Are you using the view for readability, security, or performance?', '你使用视图是为了可读性、权限，还是性能？'),
      c('OR REPLACE, TEMP, and Nested Views', 'OR REPLACE、TEMP 与视图套视图', 'SQLite can replace, temporary-scope, or stack view definitions.', 'SQLite 可以替换、临时化或叠加视图定义。', 'CREATE OR REPLACE VIEW (SQLite 3.35+) updates a view definition without a separate DROP. CREATE TEMP VIEW creates a session-scoped temporary view. Views may select from other views, which is useful for layered report interfaces. Remove a view with DROP VIEW IF EXISTS name.', 'CREATE OR REPLACE VIEW（SQLite 3.35+）可直接更新视图定义，不必先单独 DROP。CREATE TEMP VIEW 创建会话级临时视图。视图可以从其他视图 SELECT，便于分层报表接口。删除用 DROP VIEW IF EXISTS name。', 'CREATE OR REPLACE VIEW v_emp_names AS\nSELECT emp_id, first_name, last_name FROM employees;\n\nCREATE TEMP VIEW temp_high_salary AS\nSELECT * FROM v_employee_dept WHERE salary > 80000;', 'Do you need a permanent report interface or a temporary helper?', '你需要永久报表接口，还是临时辅助视图？'),
    ],
    pitfalls: { en: ['A normal view does not store a fresh copy of data.', 'Changing a view definition can break consumers.', 'TEMP VIEW lasts only for the session.'], zh: ['普通视图不存储一份新数据。', '修改视图定义可能破坏使用者。', 'TEMP VIEW 只在当前会话有效。'] },
  ...
  },
  {
    ...
    minutes: 38,
    objectives: {
      en: ['Explain ACID and TCL controls', 'Use BEGIN, COMMIT, ROLLBACK, and SAVEPOINT', 'Understand SQLite autocommit'],
      zh: ['解释 ACID 与 TCL 控制', '使用 BEGIN、COMMIT、ROLLBACK、SAVEPOINT', '理解 SQLite 自动提交'],
    },
    concepts: [
      c('ACID and TCL', 'ACID 与 TCL', 'Transactions aim for ACID; TCL statements control them.', '事务追求 ACID；TCL 语句控制事务。', 'ACID stands for Atomicity (all or nothing), Consistency, Isolation, and Durability. Transaction Control Language (TCL) includes BEGIN/START TRANSACTION, COMMIT, ROLLBACK, and SAVEPOINT. On quizzes, A in ACID means Atomicity. These controls are separate from DML row changes and DDL structure changes.', 'ACID 表示 Atomicity（原子性，要么全成要么全撤）、Consistency（一致性）、Isolation（隔离性）、Durability（持久性）。事务控制语言（TCL）包括 BEGIN/START TRANSACTION、COMMIT、ROLLBACK、SAVEPOINT。考题中 ACID 的 A 是原子性。这些控制不同于改行的 DML 和改结构的 DDL。', '-- TCL: BEGIN / COMMIT / ROLLBACK / SAVEPOINT\n-- Goal: Atomicity Consistency Isolation Durability', 'Which ACID letter fails if only half the writes succeed?', '如果只成功一半写入，破坏的是 ACID 中哪一个字母？'),
      c('All or Nothing', '要么全成，要么全撤', 'A transaction groups statements into one unit of work.', '事务把多条语句组成一个工作单元。', 'If an order reduces product stock and inserts order_items, those writes should succeed together or fail together. BEGIN starts the unit, COMMIT makes it permanent, and ROLLBACK cancels it. On failure, prefer ROLLBACK rather than committing a partial state.', '如果一张订单既减少库存又插入 order_items，这些写入应一起成功或一起失败。BEGIN 开始工作单元，COMMIT 使其永久生效，ROLLBACK 撤销。失败时应 ROLLBACK，而不是提交半成状态。', 'BEGIN;\nUPDATE products SET stock_qty = stock_qty - 1 WHERE product_id = 1;\nINSERT INTO order_items (item_id, order_id, product_id, quantity, unit_price)\nVALUES (99, 101, 1, 1, 1299.99);\nCOMMIT;', 'What statements must succeed as a set?', '哪些语句必须作为整体成功？'),
      c('SAVEPOINT and Nested Rollback', 'SAVEPOINT 与部分回滚', 'SAVEPOINT marks a point inside a transaction you can roll back to.', 'SAVEPOINT 在事务内标记可回滚的点。', 'SAVEPOINT sp1 creates a named marker. ROLLBACK TO sp1 undoes work after that marker but keeps earlier work in the same transaction. This is useful when one risky step should be undone without cancelling the whole unit. SQLite supports SAVEPOINT / ROLLBACK TO.', 'SAVEPOINT sp1 创建命名标记。ROLLBACK TO sp1 撤销该标记之后的工作，但保留同一事务中更早的工作。适合撤销某一步冒险操作而不取消整个事务。SQLite 支持 SAVEPOINT / ROLLBACK TO。', 'BEGIN;\nSAVEPOINT sp1;\nUPDATE products SET stock_qty = stock_qty - 5 WHERE product_id = 1;\nROLLBACK TO sp1;\nCOMMIT;', 'Do you need to undo everything, or only work after a marker?', '你需要撤销全部，还是只撤销某个标记之后的工作？'),
    ...
    pitfalls: { en: ['Long transactions can hold locks.', 'Do not forget COMMIT when changes should persist.', 'On failure, ROLLBACK instead of committing partial work.'], zh: ['长事务可能持有锁。', '需要保留修改时不要忘记 COMMIT。', '失败时应 ROLLBACK，不要提交半成工作。'] },
